...
### New: auto-generate recommended shifts

[lib/shiftGenerator.ts](src/lib/shiftGenerator.ts) — reads station hourly demand + the shop operating window and emits Shift records sized to keep every span ≤ `standardDailyHrsCap`. Deterministic algorithm: contiguous demand windows of length L become 1 anchor / 2 staggered / 3 overlapping shifts (L ≤ cap / ≤ 2× / > 2×). Sized against `max(peak, holiday)` per hour so generated shifts cover both day types.

- **ShiftsTab** new violet "Auto-Generate Shifts" button next to "New Shift Code". Output is additive — generated shifts carry `Shift.autoGenerated: true` so the table row paints them in violet with a "Sparkles · Auto" chip; editing any field and saving in `ShiftModal` strips the flag (the record becomes user-curated and survives a "Clear all auto-generated" sweep). [AutoGenerateShiftsModal](src/components/AutoGenerateShiftsModal.tsx) shows a 24-hour demand sparkline (normal vs peak) plus the proposed shifts with recommended-HC per day type before the user commits.
- **Type change**: `Shift.autoGenerated?: boolean` (optional, backward-compat with pre-v5.18 saves).
- **Tests**: 8 new in [shiftGenerator.test.ts](src/lib/__tests__/shiftGenerator.test.ts) covering single-anchor, two-staggered, three-window splitting, code-collision avoidance, hourly-demand profile reading, and disjoint open windows.

### New: "Plan everything" wizard

[components/PlanEverythingWizard.tsx](src/components/PlanEverythingWizard.tsx) — chains the four planning surfaces into a single 4-step flow so the supervisor doesn't have to walk Stations → Shifts → Schedule → Workforce by hand each month:

1. **Demand tuning** — runs `suggestHourlyDemandFromHistory` against every station and surfaces refreshed profiles for one-pass apply.
2. **Shift generation** — feeds the (possibly just-updated) station demand into `generateOptimalShifts`; review and apply.
3. **Auto-schedule** — kicks off the existing auto-scheduler in preserve-mode (manually-painted cells survive).
4. **Recap** — surfaces coverage diagnostics + which steps the user actually applied vs skipped.

Each step is skippable. Launched from a violet "Plan everything" button on the Schedule toolbar (`schedule.planEverything`).

### New: suggest hourly demand from history

[lib/demandHistory.ts](src/lib/demandHistory.ts) — reverses the data flow: instead of the supervisor typing per-hour HC by intuition, the helper walks `allSchedules`, observes per-hour coverage at a station across normal vs peak days, and returns recommended `HourlyDemandSlot[]`. Uses `ceil(observedCoverage / distinctDays)` so under-staffing isn't masked. Skips closed days (don't drag the average down). Detects cross-midnight shifts (counts the active calendar day's hours only).

- **StationModal** new violet "Suggest from history" panel inside the hourly-demand editor. Diagnostics line shows `{months} month(s) · {normal} normal day(s) · {peak} peak day(s)` so the supervisor knows what data it was working from. Disabled when the station has no saved id (the helper keys off `station.id`).
- **App.tsx** wires `handleSuggestStationDemandFromHistory` closure (modal owns UI; App owns data). Bulk-apply via `handleApplyStationDemandBulk` for the wizard's pass.
- **Tests**: 10 new in [demandHistory.test.ts](src/lib/__tests__/demandHistory.test.ts).

### New: holiday-tier hourly demand

`Station` gains `holidayMinHC?: number` and `holidayHourlyDemand?: HourlyDemandSlot[]`. Festival days (Eid, Ashura) often need staffing levels that diverge from a normal peak — either much higher (full-day surge) or much lower (shop closed entirely). When configured, the new tier WINS over peak on holiday dates; when unset, holidays fall back to peak (preserves pre-v5.18 behaviour).

- `getRequiredHC(station, hour, isPeakDay, isHoliday?)` — new optional `isHoliday` arg consults the holiday tier first, falls through to peak otherwise.
- StationModal renders a third tier with a "Copy from peak" shortcut + flat fallback input.
- Auto-shift generator sizes against `max(peak, holiday)` so generated shifts cover both day types from one library.

### New: workforce planning is leave-aware

`MonthlyPlanSummary` gains `plannedLeaveFTELoss`, `plannedLeaveBreakdown` (annual / sick / maternity), and `affectedEmployeeCount`. Pre-v5.18 the recommended-FTE headline assumed every contracted employee was working all month; in August (when 4 employees plan annual leave) that overstates available roster by ~2 FTE. The drill-month view in WorkforcePlanningTab now surfaces an amber banner when planned leave reduces effective HC, with the per-type split. Sum-of `(leaveDaysInMonth / daysInMonth)` per employee — same days the auto-scheduler treats as unavailable.

### New: hiring lead-time

`Config.hiringLeadTimeWeeks?: number` — operator-set in a new VariablesTab section. Drives:
- **StaffingAdvisoryCard**: a "Hiring lead time" hint surfaces when the active mode recommends ≥1 hire — *"Post {hires} job(s) now to have them productive by {today + N weeks}"*.
- **WorkforcePlanningTab** hiring roadmap: passes `Math.ceil(weeks / 4)` as `leadMonths` so the roadmap shifts forward to land hires in time for their target demand month.

### New: cost-curve slider in StaffingAdvisoryCard

What-if hire-count slider that scrubs between 0 and the active mode's recommendation, updating OT savings, fines avoided, salary added, and net delta on drag. Linear interpolation against the active mode (the existing Validate button still runs the full simulation for ground truth). Reset returns to the recommended position. The KPI card stays the source of truth; the slider is a "what would 3 hires get me instead of 5?" view that never required leaving the card.

### New: coverage diagnostics ("Why didn't you cover X?")

[lib/coverageDiagnostics.ts](src/lib/coverageDiagnostics.ts) — post-processes any schedule, classifies every unfilled (station, day, hour) tuple as one of:
- `no-eligible-employees` — nobody in the roster lists this station.
- `all-eligible-on-leave` — every eligible employee has a leave cell that day.
- `all-eligible-already-scheduled` — every eligible employee is on a different work shift / hit a cap.
- `all-eligible-fixed-rest` — every eligible employee has fixedRestDay matching this DOW.
- `station-closed` / `unknown` — fall-throughs for sanity.

[CoverageDiagnosticsPanel](src/components/Schedule/CoverageDiagnosticsPanel.tsx) sits below the schedule grid, collapsible, auto-hides when fully covered. Each row groups consecutive hours at the same (station, day) so a "11:00–17:00 short by 6 PAX-hour(s)" line replaces six separate per-hour rows. Tests: 6 new in [coverageDiagnostics.test.ts](src/lib/__tests__/coverageDiagnostics.test.ts).

### UX hardening pass

- **Modal dirty-state guard** — `useModalKeys` accepts an optional `canClose` predicate; EmployeeModal / ShiftModal / HolidayModal track JSON-baseline dirty state and route Esc / X / Cancel through a `useConfirm()` "discard changes?" dialog. Pre-v5.18 a stray Esc lost 10 minutes of form entry silently.
- **Required-field indicators + numeric constraints** — `SettingField` gains `required` (red asterisk + `aria-required`), `min` / `max` / `step` (forwarded to the underlying input), and inline `error` rendering. Applied to EmployeeModal (id / name / role / weekly hours / salary), ShiftModal (code / name / start / end / duration / break), HolidayModal (date / name).
- **Schedule grid: per-cell violation markers** — App.tsx memoizes `violationCellKeys` from `violations` and threads it through ScheduleTab → row → ScheduleCell, which paints a small red corner dot. The supervisor spots violations during paint without leaving the tab.
- **Schedule grid: keyboard navigation** — every cell has `tabIndex={readOnly ? -1 : 0}`, `data-cell-emp` / `data-cell-day` attributes, focus-visible outline, and arrow-key handlers that move focus between siblings via DOM querying. Enter/Space activates a paint click.
- **Online-mode save indicator** — the auto-save effect bailed out under `isAuthenticated`, so the Save Draft badge in the schedule toolbar showed "Last saved: never" forever in Online mode. `updateActive`'s Firestore sync path now drives `saveState` (saving → saved/error) so the badge tracks the actual write outcome.
- **Firestore error toasts** — non-quota sync failures used to console.error silently. Routed through a `syncErrorState` ring buffer that the existing `showInfo` machinery picks up — the user sees their edit didn't land instead of assuming it did.
- **Print schedule legend** — printed pages used to show shift codes (M1/N2) with no key. Added a legend table below the schedule with code / name / hours / duration; only includes codes actually used in the month so it stays compact. Hardcoded English "Iraqi Labor Scheduler" company-name fallback now i18n'd.
- **Bulk-edit progress feedback** — BulkEditEmployeesModal disables the apply button, shows a spinner + "Applying to N…" label, and yields a frame before the heavy mutation so the spinner actually paints.
- **RTL class sweep** — 30+ `ml-/mr-/pl-/pr-/text-left/text-right` occurrences across 13 files (DashboardTab, ScheduleTab, PayrollTab, CoverageOTAnalysisTab, LeaveManagerModal, OnlineSetup, SchedulePreviewModal, StaffingAdvisoryCard, VariablesTab, UsersPanel, WorkforcePlanningTab, CompanySwitcher) → logical `ms-/me-/ps-/pe-/text-start/text-end`. Arabic mode now lays out correctly across these surfaces.
- **Payroll leave-range tooltip i18n** — pre-v5.18 the tooltip joined raw `${r.type}: …` strings (English `annual` / `sick` / `maternity` even in Arabic mode); now formatted via `payroll.leaveRange.line` + per-type i18n keys.

### Compatibility

- **All 253 tests pass** (16 new across `shiftGenerator.test.ts`, `demandHistory.test.ts`, `coverageDiagnostics.test.ts`).
- `tsc --noEmit` clean. Vite production build clean (1.012MB main chunk, gz 280KB — pre-existing chunk-size warnings only). Secret-leak audit clean (no PEMs, no Firebase keys, no service-account JSONs, `.env.local` gitignored).
- **Type changes are additive**: `Shift.autoGenerated`, `Station.holidayMinHC` / `holidayHourlyDemand`, `Config.hiringLeadTimeWeeks`, `MonthlyPlanSummary.plannedLeaveFTELoss` / `plannedLeaveBreakdown` / `affectedEmployeeCount`. All optional; pre-v5.18 saves load unchanged.
- **`getRequiredHC` API change is backward-compatible**: callers omitting the new `isHoliday` arg get pre-v5.18 behaviour (holidays treated as peak).
- **Dual-mode parity preserved** — every new mutator (`handleApplyGeneratedShifts`, `handleClearAutoGeneratedShifts`, `handleApplyStationDemandBulk`) routes through `setShifts` / `setStations` / `updateActive` so Offline + Online land in identical end state and audit emits a single coherent edit per apply.
