...
// In dev, redirect Electron's userData (localStorage, IndexedDB, cookies,
// Firebase Auth tokens, mode-picker choice, …) to a sandboxed folder in the
// repo. Without this, dev sessions share state with the user's installed
// production app — picking a mode or signing into Firebase in dev would
// also apply to the v3.0.0 install. The folder is gitignored. Must run
// BEFORE any code that reads getPath('userData').
if (isDev) {
  const devUserData = path.join(__dirname, '..', '.dev-userdata');
...
if (!gotTheLock) {
  ...
  // v5.40.0 — AI Services (beta) was removed. Best-effort cleanup of the
  // encrypted OpenRouter keys + consent records it stored under
  // <userData>/ai/. Harmless no-op once the folder is gone.
  try {
    fs.rmSync(path.join(app.getPath('userData'), 'ai'), { recursive: true, force: true });
  } catch { /* best-effort — leftover encrypted blobs are inert */ }
