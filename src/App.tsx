...
import { optimizeForLiability } from './lib/liabilityOptimizer';
...
export default function App() {
  ...
  // v5.19.2 — wholesale library swap from the Optimize tab. Removes
  // the supplied work shifts, then appends the proposal. System
  // shifts (CP/OFF/AL/SL/MAT/PH) are never affected — the modal
  // pre-filters them out. Wrapped in a confirm modal so the
  // supervisor sees the impact before committing.
  const handleReplaceLibrary = React.useCallback((toDelete: Shift[], toAdd: Shift[]) => {
    if (toDelete.length === 0 && toAdd.length === 0) return;

    // Count how many delete-candidates are referenced in any saved
    // schedule (active month + history). Cells referencing a deleted
    // shift code become dangling references; the migration script
    // doesn't auto-resolve those, so we surface a warning that
    // includes the count.
    const deleteSet = new Set(toDelete.map(s => s.code));
    ...
    for (const monthSched of Object.values(allSchedules || {})) {
      for (const empSched of Object.values(monthSched)) {
        for (const entry of Object.values(empSched)) {
          if (entry?.shiftCode && deleteSet.has(entry.shiftCode)) referencedCells++;
        }
      }
    }
    ...
    const doSwap = () => {
      setShifts(prev => {
        const haveCodes = new Set(prev.map(s => s.code.toUpperCase()));
        const filtered = prev.filter(s => !deleteSet.has(s.code));
        const additions = toAdd.filter(g => !haveCodes.has(g.code.toUpperCase()));
        return [...filtered, ...additions];
      });
    };

    const messageKey = referencedCells > 0
      ? 'confirm.replaceLibrary.bodyWithReferences'
      : 'confirm.replaceLibrary.body';
    setConfirmState({
      isOpen: true,
      title: t('confirm.replaceLibrary.title'),
  ...
  const handleRunAutoScheduler = (mode: 'fresh' | 'preserve' = 'fresh', range?: { start: string; end: string }) => {
    try {
      ...
      while (cursor.getTime() <= stopMonth.getTime()) {
        ...
        const { schedule: rawMonthOut, updatedEmployees, compDayShortfall } = runAutoScheduler({
        ...
        // v5.19.0 — liability-aware post-pass. Default-on; supervisors
        // can opt out via Config.liabilityAwarePass = false to inspect
        // the raw greedy-fill output. Swaps reduce over-cap OT, even
        // out holiday-premium spread, and close comp-day rotation gaps
        // without breaking any cap or coverage constraint.
        const monthOut = (config.liabilityAwarePass ?? true)
          ? optimizeForLiability({
              schedule: rawMonthOut,
  ...
  const runSingleMonthAuto = (mode: 'fresh' | 'preserve', range?: { startDay: number; endDay: number }) => {
    ...
    // v5.19.0 — liability-aware post-pass. See multi-month branch above
    // for rationale. The optimiser is HC-neutral (every swap preserves
    // coverage) so the preview totals computed below stay accurate.
    const liabilityResult = (config.liabilityAwarePass ?? true)
      ? optimizeForLiability({
          schedule: rawSchedule, employees: updatedEmployees, shifts, stations,
          holidays, config, preserveExisting: effectivePreserve,
        })
      : null;
    ...
    const stats = buildPreviewStats(
      ...
      compDayShortfall,
      liabilityResult ? {
        swapCount: liabilityResult.swaps.length,
        overCapHoursSaved: liabilityResult.totalOverCapHoursSaved,
          ...
          >
            ...
            {activeTab === 'shifts' && (
              <ShiftsTab
                ...
                onReplaceLibrary={handleReplaceLibrary}
