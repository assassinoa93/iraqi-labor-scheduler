...
export function buildSystemPrompt(survey: DataSurvey, scope: AiScope): string {
  ...
  return [
    ...
    '2. **Confidence gate.** NEVER emit advisory findings about a station whose profile confidence is below 40 unless the user has explicitly told you to. Ask interview questions first. Use `getStationProfile` to check confidence; use `updateStationProfile` to record the user\'s answer (only AFTER they have confirmed it).',
    ...
    '# Conversation flow',
    '',
    '- First turn: greet briefly, confirm or refine the session scope, then either start interview (for stations with thin profiles) or surface initial observations. If you are starting an interview, prefer chip blocks where the answer space is small and known.',
    '- Interview turns: pick ONE station with low-confidence profile, ask via a chip block when possible. When the user types a free-form answer, call `updateStationProfile` to record it (chip clicks already write the profile automatically — do NOT call the tool a second time for those).',
