...
import { X, AlertCircle, Lock, Sparkles } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={shift ? t('modal.shift.title.edit') : t('modal.shift.title.new')}>
      ...
      >
        <div className="p-6 border-b border-slate-100 dark:border-slate-700/60 flex justify-between items-center bg-slate-50 dark:bg-slate-800/40">
          ...
          <button ref={closeButtonRef} onClick={requestClose} aria-label={t('action.cancel')} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors">
        ...
        <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <SettingField required label={t('modal.shift.field.code')} value={formData.code} onChange={v => setFormData({...formData, code: v})} />
            <SettingField required label={t('modal.shift.field.name')} value={formData.name} onChange={v => setFormData({...formData, name: v})} />
            <SettingField required label={t('modal.shift.field.start')} type="time" value={formData.start} onChange={v => setFormData({...formData, start: v})} />
            <SettingField required label={t('modal.shift.field.end')} type="time" value={formData.end} onChange={v => setFormData({...formData, end: v})} />
            <SettingField min={0} max={24} step={0.25} label={t('modal.shift.field.duration')} type="number" value={formData.durationHrs} onChange={v => setFormData({...formData, durationHrs: Math.max(0, Math.min(24, parseFloat(v) || 0))})} />
            <SettingField min={0} max={240} step={5} label={t('modal.shift.field.break')} type="number" value={formData.breakMin} onChange={v => setFormData({...formData, breakMin: Math.max(0, Math.min(240, parseInt(v) || 0))})} />
          </div>
