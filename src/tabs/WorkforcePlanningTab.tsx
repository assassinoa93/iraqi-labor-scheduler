...
import { AnnualHeadcountPanel } from '../components/Planning/AnnualHeadcountPanel';
...
interface Props {
  ...
  onGoToRoster: () => void;
  onGoToLayout: () => void;
  // v5.22.0 — config mutator for the "What's new" inline banner that
  // offers one-click adoption of realistic coverage. Optional so test
  // fixtures and read-only views still work; the banner stays hidden
...
export function WorkforcePlanningTab(props: Props) {
  ...
  // v5.22.0 — forecast year stepper is folded behind a small "Forecast"
  // affordance so users planning the current year (the 95% case) don't
  // see a year stepper they don't need. Auto-expands when forecasting
  // a non-current year so the indicator stays visible.
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row items-start justify-between gap-4 mb-2">
        <div className="flex items-center gap-4 flex-wrap">
          {/* v5.22.0 — year selector. Collapses to a static "current
              year" pill by default; the stepper appears when the user
              clicks "Forecast different year" or when they're already
              viewing a non-current year (so the indicator never
              disappears mid-forecast). Big visual saving on the
              every-day case. */}
          {forecastOpen || isForecasting ? (
            <div className="bg-white px-2 py-2 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-1">
              ...
              {isForecasting && (
                <button
                  onClick={() => { setForecastYear(config.year); setForecastOpen(false); }}
            ...
            <button
              ...
              className="bg-white px-3 py-2 rounded-2xl border border-slate-200 shadow-sm text-[10px] font-black uppercase tracking-widest text-slate-600 hover:text-slate-900 hover:border-slate-300 transition-all flex items-center gap-2"
              title={t('workforce.forecast.show')}
            >
              <Calendar className="w-3.5 h-3.5" />
              {t('workforce.forecast.show')}
            </button>
          )}
      ...
      {/* v5.22.0 — "What's new" banner: realistic coverage. Shown to
          legacy users still on `coverageMode === 'simple'` who haven't
          dismissed the tip. Offers one-click adoption + a "Maybe later"
          dismiss. Hidden once dismissed (the dismissal persists in
          `config.dismissedTips`) or once the user adopts realistic
          coverage. Hidden if `setConfig` isn't wired (read-only views). */}
      {setConfig
        && (config.coverageMode ?? 'simple') === 'simple'
        && !(config.dismissedTips ?? []).includes('realistic-coverage-v5.21')
        && (
                ...
                <div className="flex items-center gap-2 mt-3">
                  <button
                    onClick={() => setConfig(prev => ({
                      ...
                      dismissedTips: [...(prev.dismissedTips ?? []), 'realistic-coverage-v5.21'],
                    }))}
                  ...
                  >
                    ...
                    {t('workforce.whatsNew.realisticCoverage.tryIt')}
                  </button>
                  <button
                    onClick={() => setConfig(prev => ({
                      ...prev,
                    ...
                    className="px-3 py-1.5 text-emerald-800 dark:text-emerald-200 text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100/50 dark:hover:bg-emerald-500/10 rounded-lg transition-all"
                  >
                    {t('workforce.whatsNew.realisticCoverage.maybeLater')}
                  </button>
                </div>
              </div>
            </div>
