...
import { readHistory } from '../lib/complianceHistory';
...
export function DashboardTab(props: DashboardTabProps) {
  ...
  // v5.41.0 (F3) — feed the violations KPI a 30-day sparkline + delta from the
  // same localStorage history the ComplianceTrendCard records (no server work,
  // dual-mode safe). Short/empty history degrades gracefully: the sparkline
  // hides (<2 points) and the delta renders a muted "no prior month" chip.
  const kpiHistory = useMemo(
    // readHistory reads localStorage keyed only by company; the live violation
    // delta is computed separately below off the current value, so the history
    // series only needs to refresh when the active company changes.
    () => readHistory(activeCompanyId),
    [activeCompanyId],
  );
  const violationsSpark = useMemo(() => kpiHistory.map((s) => s.violations), [kpiHistory]);
  const violationsDelta = useMemo(
    () => computeDelta(totalViolationInstances, kpiHistory[0]?.violations ?? 0, kpiHistory.length >= 2),
    [kpiHistory, totalViolationInstances],
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-2">
        ...
        <div className="flex bg-white dark:bg-slate-900 p-1 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-slate-800 text-white rounded-xl text-[11px] font-semibold tracking-tight hover:bg-slate-800 dark:hover:bg-slate-700 transition-all shadow-md active:scale-95"
          >
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" role="dialog" aria-modal="true" aria-label={t('dashboard.stats.title')}>
            ...
            >
              ...
              <div className="p-6 bg-slate-50 dark:bg-slate-800/40 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                ...
                <button onClick={() => setIsStatsModalOpen(false)} className="bg-slate-900 dark:bg-slate-700 text-white px-8 py-3 rounded-2xl font-semibold text-xs tracking-tight shadow-xl hover:bg-slate-800 dark:hover:bg-slate-600 transition-all">{t('dashboard.stats.close')}</button>
              </div>
            </motion.div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-4 border-y border-white/10">
                ...
                <div>
                  ...
                  {/* v5.41.0 (03.5) — abbreviate to keep the figure inside its
                      column; full grouped value on hover. */}
                  <p className="text-xl font-black text-rose-400 tabular-nums" title={`${fmt.num(Math.round(totalOTPay))} IQD`}>{fmt.abbrev(totalOTPay).short} IQD</p>
                ...
                <div>
                  ...
                  {/* v5.41.0 (03.5) — the net delta is the figure that clipped;
                      abbreviate to K/M/B, full value on hover. */}
                  <p
                    className={cn(
                      "text-xl font-black tabular-nums",
                      netMonthlyDelta > 0 ? "text-emerald-400" : netMonthlyDelta < 0 ? "text-amber-400" : "text-slate-200"
                    )}
                    title={`${netMonthlyDelta >= 0 ? '+' : '−'}${fmt.num(Math.abs(Math.round(netMonthlyDelta)))} IQD`}
                  >
                    {netMonthlyDelta >= 0 ? '+' : '−'}{fmt.abbrev(Math.abs(netMonthlyDelta)).short} IQD
                  </p>
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6 bg-blue-50/50 dark:bg-blue-500/10 border-blue-100 dark:border-blue-500/30">
              ...
              <button onClick={onGoToRoster} className="w-full py-2 bg-blue-600 text-white rounded text-[11px] font-semibold tracking-tight hover:bg-blue-700 transition-all">{t('dashboard.recruitment.cta')}</button>
            </Card>
          </div>
        </div>
      )}
      ...
      {employees.length === 0 && (
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center pt-4">
            <button onClick={onGoToRoster} className="px-8 py-3 bg-slate-900 dark:bg-slate-700 text-white rounded-xl font-semibold text-xs tracking-tight hover:bg-slate-800 dark:hover:bg-slate-600 transition-all shadow-xl flex items-center gap-2">
              <Plus className="w-4 h-4" />
              {t('dashboard.empty.create')}
            </button>
            <button onClick={onLoadSample} className="px-8 py-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-xl font-semibold text-xs tracking-tight hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-all flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <KpiCard label={t('dashboard.kpi.workforce')} countTo={employees.length} unit={t('kpi.unit.staff')} />
        <KpiCard
          label={t('dashboard.kpi.violations')}
          countTo={totalViolationInstances}
          trend={violations.length > 0 ? 'Critical' : 'Perfect'}
          delta={violationsDelta}
          deltaProps={{ lowerIsBetter: true }}
          spark={violationsSpark}
          sparkTone={violationsDelta.direction === 'up' ? 'rose' : violationsDelta.direction === 'down' ? 'emerald' : 'slate'}
        />
        <KpiCard label={t('dashboard.kpi.stations')} countTo={stations.length} unit={t('kpi.unit.stations')} />
        <KpiCard label={t('dashboard.kpi.compliance')} countTo={complianceScoreValue} countSuffix="%" trend={complianceScoreValue < 80 ? 'Critical' : 'Health'} />
        <KpiCard
