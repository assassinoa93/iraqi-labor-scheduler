...
## 🌟 Key Features
- **🌐 Bilingual UI (English / Arabic)**: One-click language toggle in the sidebar with full RTL layout for Arabic.
...
- **👁️ Schedule Preview & Undo**: Review the auto-scheduler's proposed assignments, hours, and compliance impact before applying. One-step undo lets you revert the most recent apply.
- **📊 Smart Staffing Advisory**: Coverage gaps now break down by role — see exactly how many cashiers, operators, or drivers you need to hire to close peak-hour shortfalls.
...
2. Under the **latest release (v1.4.0)**, scroll down to the **Assets** section.
3. Download `Iraqi-Labor-Scheduler-Setup-1.4.0.exe` **and** `SHA256SUMS.txt`.
4. (Optional but recommended) Verify the installer hash — open PowerShell in the folder where you saved both files and run:
   ```powershell
   Get-FileHash -Algorithm SHA256 .\Iraqi-Labor-Scheduler-Setup-1.4.0.exe
   ```
