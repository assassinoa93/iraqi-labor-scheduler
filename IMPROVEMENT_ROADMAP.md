...
> **Implementation status (v5.27.0, branch `feature/data-trust-batch`, 7 commits, tsc clean, 443 tests):**
> - ✅ **Batch 1 (Data Trust & Safety) shipped** — leave-queue persistence, delete-orphan fixes, online backup-import guard, immutable-audit Clear guard, modal ID validation, StationModal dirty-guard, real Reports coverage tile, PDF Arabic-tofu fix. ("Confirm before Rebuild From Scratch" intentionally **skipped** — preview-and-apply + 5-deep undo already protect it.)
> - ✅ **Batch 2 (UX) shipped (8.5/9)** — FindingsList search + expand/collapse-all; Roster leave chips + Dashboard "Time Off (7 days)" card; "Copy last month" + "Repeat week 1"; unified SetupChecklist + wired Plan-Everything wizard; station picker in the paint palette; bulk "Record Leave"; fixed-term contract end dates + expiry surfacing. (**Deferred:** period-over-period deltas.)
> - ◻️ **Batch 3 (started)** — ✅ aria-sort on sortable tables, ✅ prefers-reduced-motion. Remaining: compliance depth, focus-trap, grid ARIA, i18n/RTL, performance, and data-trust leftovers (see below). Items flagged "confirm article number with counsel" (Art. 69 break, 21-day minimum) are **blocked on legal confirmation**.
