...
> **Implementation status (v5.34.0 shipped; 476 tests, tsc + build clean):**
> - ✅ **Batches 1–3 shipped across v5.27 → v5.34** — data-trust (leave-queue persistence, delete-orphan fixes, online guards, ID validation, dirty-guard, real coverage tile, PDF tofu); UX (FindingsList search, Roster/Dashboard leave visibility, Copy-month/Repeat-week, unified checklist + wired wizard, station picker, bulk leave, contract/probation dates + CSV import); compliance depth (Art. 69 break check, Art. 71 21-day minimum, annual-leave overdraw warning, statute citations, probation awareness); a11y (aria-sort, prefers-reduced-motion, focus-trap + focus-restore, cell aria encoding violation/diff beyond colour, colour-blind diff glyphs, form-modal backdrop guard); i18n/RTL (grid dates + stats tooltip, RTL arrows, scattered labels); perf (auto-save serialization defer, otSummary gating, Roster pagination, batch drag-paint, non-blocking auto-scheduler spinner); **period-over-period deltas** (v5.34).
> - ◻️ **Still pending** — PDF report → canonical pay engine (changes displayed numbers; needs user verification — `computePayrollRow` already extracted in v5.34 to enable it); full schedule-grid `role=grid` ARIA Tier B (needs screen-reader testing); online hydration skeleton (online-only verification); `holidayMinHC` in coverage scenarios (low value); OnlineSetup screen localization. "Confirm before Rebuild From Scratch" intentionally **skipped** (preview-and-apply + 5-deep undo already protect it).
